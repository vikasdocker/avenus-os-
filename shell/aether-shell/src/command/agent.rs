...
static AGENT_METADATA: Lazy<CommandMetadata> = Lazy::new(|| CommandMetadata {
    ...
    usage: "agent <status|sessions|inspect|intent|cancel> [args]".to_string(),
    ...
    async fn execute(
        ...
        if args.is_empty() {
            return Err(anyhow::anyhow!(
                "agent requires subcommand: status, sessions, inspect, intent, cancel"
            ));
        }
        ...
        match args[0] {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "Unknown agent subcommand: {}. Use: status, sessions, inspect, intent, cancel, audit",
                    args[0]
                ));
            }
        }
